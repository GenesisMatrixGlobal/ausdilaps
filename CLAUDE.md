...
- **Markup and Measure (`site-markups`, code `SMK`) — three tabs, two different kinds of map.** Residential Mark Up and Road Markup are Google **Static Maps** PNGs with an SVG overlay: no panning, and the zoom slider re-fetches the image. That's right for producing a fixed export and wrong for measuring, so the third tab, **Measure**, is a real **Maps JavaScript API** map. See `docs/measure-tab.md`.
