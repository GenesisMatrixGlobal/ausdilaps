...
- **Tender Watch (BUILT — don't redo):** the nightly tender pipeline. `/staff/accounts/tools/tender-watch` (no `/admin` route — admins reach it there and get the operator panels via the `isAdmin` flag). See `docs/tender-watch.md`.
  ...
  - ⚠️ **TenderSearch bulletins parse as N copies of the whole digest** — each of ~14 links becomes an item whose `excerpt` is the entire bulletin, so titles come out as tracking URLs and one bulletin is classified 14 times at Opus rates. `displayTitle()`/`groupKey()` in `group.ts` are read-time workarounds, marked as such. The real fix is per-notice splitting in `parseDigest()`; needs a real `.eml`. See `docs/tender-watch.md`.
