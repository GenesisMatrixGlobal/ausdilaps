...
- **Knowledge base (BUILT — Phase 1 only):** the corpus behind the department search bar. Migration `0009`, `lib/knowledge/*`, `/staff/<dept>/training/manage`.
  ...
  - `npm run index:training` indexes `content/training/**/*.mdx` into the same corpus. Idempotent (upserts on `source_ref`). **Not yet wired into the deploy** — run it after adding a module.
  - **Phase 2 (NOT built): retrieval + answers.** Postgres FTS first, deliberately no embeddings — the exact Phase 4 pgvector migration is written in a comment in `0009`. See the plan at `~/.claude/plans/lets-plan-these-tools-serene-iverson.md`.
