...
import { coverFrameBounds, coverViewFor } from "@/lib/cover-photo/frame";
...
// The metre floor: a tiny lot must not produce a frame tighter than a wide one.
const tiny = coverViewFor(lot(CITIES[0][1], 9, 12))!;
const suburban = coverViewFor(lot(CITIES[0][1], 15, 45))!;
ok(
  Math.abs(mercatorSpan(tiny).spanX - mercatorSpan(suburban).spanX) < 1e-9,
  "the metre floor should pin both a 9x12 and a 15x45 lot to the same minimum frame width"
);
